...
| 参数 | 类型 | 说明 |
|------|------|------|
| message | 1xN logical/数值数组 | 信息比特序列，N须为k的整数倍，k = 2^r - 1 - r |
| r | 正整数 | 校验比特数，默认 3（即 Hamming(7,4)） |

**输出参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| codeword | 1xM 数组 | 编码后比特序列，M = N/k * n，n = 2^r - 1 |
| G | k x n 矩阵 | 生成矩阵，系统形式 [I_k \| P] |
| H | r x n 矩阵 | 校验矩阵，系统形式 [P' \| I_r] |
...
| 参数 | 类型 | 说明 |
|------|------|------|
| received | 1xM 数组 | 接收到的比特序列，M须为n的整数倍，n = 2^r - 1 |
| r | 正整数 | 校验比特数，默认 3 |
...
| 参数 | 类型 | 说明 |
|------|------|------|
| decoded | 1xN 数组 | 译码后的信息比特序列，N = M/n * k |
| num_corrected | 整数 | 纠正的错误比特总数 |
...
| 参数 | 类型 | 说明 |
|------|------|------|
| message | 1xN logical/数值数组 | 信息比特序列 |
| gen_polys | 1xn 数组 | 生成多项式，八进制表示，默认 [171, 133]（NASA标准码，码率1/2，K=7） |
| constraint_len | 正整数 | 约束长度K，默认 7 |

**输出参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| coded | 1x((N+K-1)*n) 数组 | 编码后比特序列，包含K-1个尾比特使编码器归零 |
| trellis | 结构体 | 网格结构体，供Viterbi译码使用，含字段：`.numStates`(状态总数)、`.n`(每输入比特对应输出比特数)、`.K`(约束长度)、`.nextState`(numStates x 2 状态转移表)、`.output`(numStates x 2 输出表) |
...
| 参数 | 类型 | 说明 |
|------|------|------|
| received | 1xM 数组 | 接收比特/软值序列。硬判决：二进制数组；软判决：实数数组（正值倾向1，负值倾向0） |
| trellis | 结构体 | 网格结构体（由 conv_encode 生成） |
| decision_type | 字符串 | 判决类型，'hard'(默认，汉明距离) 或 'soft'(欧氏距离) |
...
|------|------|------|
| message | 1xN logical/数值数组 | 信息比特序列 |
| num_iter_hint | 正整数 | 建议译码迭代次数，默认 6，仅记录到params供译码参考 |
| interleaver_seed | 正整数 | 交织器随机种子，默认 0，编解码须一致 |

**输出参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| coded | 1x3N 数组 | 编码后比特序列，排列顺序 [系统位, 校验位1, 校验位2]，各N个比特 |
| params | 结构体 | 编码参数（供 turbo_decode 使用），含字段：`.msg_len`(N)、`.interleaver`(交织索引)、`.deinterleaver`(解交织索引)、`.num_iter`(建议迭代次数)、`.fb_poly`(反馈多项式)、`.ff_poly`(前馈多项式)、`.constraint_len`(RSC约束长度) |

---

### turbo_decode.m

**功能**：Turbo迭代译码器（基于Max-Log-MAP/BCJR算法）

**输入参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| received | 1x3N 实数数组 | 接收软值序列，排列顺序 [系统位, 校验位1, 校验位2]，正值倾向比特1 |
| params | 结构体 | 编码参数结构体（由 turbo_encode 生成） |
| snr_db | 标量 | 信噪比 (dB)，用于计算信道可靠度 Lc |
| num_iter | 正整数 | 迭代次数，默认使用 params.num_iter |

**输出参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| decoded | 1xN 二进制数组 | 硬判决译码结果 |
| LLR_out | 1xN 实数数组 | 最终对数似然比，正=1，负=0 |
...
| 参数 | 类型 | 说明 |
|------|------|------|
| message | 1xN 数组 | 信息比特序列，N须为k的整数倍 |
| n | 正整数 | 码字长度，默认 64 |
| rate | 0~1之间实数 | 码率，默认 0.5，k = round(n*rate) |
| H_seed | 正整数 | 校验矩阵生成随机种子，默认 0，编解码须一致 |

**输出参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| codeword | 1xM 数组 | 编码后比特序列 |
| H | (n-k) x n 矩阵 | 稀疏二进制校验矩阵 |
| G | k x n 矩阵 | 二进制生成矩阵 |
...
**功能**：LDPC码置信传播(BP)迭代译码

**输入参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| received | 1xM 实数数组 | 接收软值序列，M须为n的整数倍，正值倾向比特1 |
| H | (n-k) x n 矩阵 | 校验矩阵（由 ldpc_encode 生成） |
| k | 正整数 | 信息位长度，用于提取系统位 |
| snr_db | 标量 | 信噪比 (dB)，用于计算初始LLR |
| max_iter | 正整数 | 最大迭代次数，默认 50 |

**输出参数**：

| 参数 | 类型 | 说明 |
|------|------|------|
| decoded | 1xN 数组 | 译码后的信息比特序列 |
| LLR_out | 1x(n*num_blocks) 数组 | 最终全码字LLR |
| num_iter_done | 1xnum_blocks 数组 | 各码块实际迭代次数 |

---

### plot_ber_curve.m

BER vs SNR曲线绘制工具，绘制多条编码方案的误码率对比曲线，含BPSK理论参考线。输入：`snr_db`(SNR数组)、`ber_data`(MxK BER矩阵)、`legend_labels`(图例)、`title_str`(标题)。

---

### test_channel_coding.m

单元测试脚本（22项），覆盖Hamming、卷积码+Viterbi、Turbo码、LDPC码和异常输入。
