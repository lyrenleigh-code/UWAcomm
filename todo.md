...
### OFDM V4.2 — static+fd=1Hz均修复 ✅（2026-04-10）

帧结构：`[HFM+|guard|HFM-|guard|LFM1|guard|LFM2|guard|data]`
TX: 06 ofdm_modulate(IFFT+CP)，与SC-FDE的区别：TX有IFFT
RX: 逐子载波MMSE-IC(不用eq_mmse_ic_fde) + LFM相位+CP盲估α
信道估计: static=OMP(V4.1), 时变=BEM(DCT), nv_post实测噪声兜底(V4.2)
...
**V4.1修复**: static高SNR发散(46-49%)——GAMP→OMP
**V4.2修复**: fd=1Hz高SNR非单调(25dB:3.81%)——nv_post实测噪声兜底nv_k防LLR过度自信
**fd=1Hz@25dB残留0.68%**: 多普勒估计仅恢复16%（α残留~0.8Hz），BEM模型极限
**fd=5Hz**: 已知物理极限（α·fc∈Jakes频谱）

### SC-TDE V5.0 — 代码完成，待测试 🔶（2026-04-09）
...
### ✅ P2→V4.2: OFDM — static+fd=1Hz均修复
- V4.1: GAMP→OMP修复static高SNR发散
- V4.2: nv_post实测噪声兜底nv_k，修复fd=1Hz高SNR非单调
- static: 0%@10dB+，fd=1Hz: 0%@15-20dB，0.68%@25dB（多普勒估计极限）

### 🔶 P3→V5.0: SC-TDE — 代码完成，待测试
- V5.0代码完成，对齐SC-FDE V4.0两级分离架构
- 训练精估替代CP精估（SC-TDE无CP结构）
- 残余CFO使用盲估alpha_est*fc
- 待MATLAB测试验证BER
...
| ✅ | **OFDM端到端V4.2** | **完成** | OMP+nv_post，static 0%@10dB+, fd=1Hz 0%@15-20dB |
| 🔶 进行 | **SC-TDE端到端V5.0改造** | **代码完成** | 去oracle dop_rate, 训练精估替代CP, 待MATLAB测试 |
