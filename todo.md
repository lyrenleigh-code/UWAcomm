...
| 13 端到端仿真 | `13_SourceCode/` | 12 | 🔶 SC-FDE V4.0+OFDM V4.3完成, SC-TDE V5.1同步修复 |
...
**V4.1**: GAMP→OMP修复static高SNR发散
**V4.2**: nv_post兜底nv_k防LLR过度自信
**V4.3**: 时变跳过CP精估(防N_blocks敏感) + 空子载波CFO + DD-BEM + Turbo10轮
**fd=1Hz ~1%地板**: Doppler相位≡Jakes相位(物理极限)，BEM跟踪残余CFO的实际上限
**fd=5Hz**: 已知物理极限（α·fc∈Jakes频谱）

### SC-TDE V5.1 — LFM同步修复 🔶（2026-04-10）

帧结构：`[HFM+|guard|HFM-|guard|LFM1|guard|LFM2|guard|data]`
RX：①LFM相位+训练精估α ②resample补偿 ③LFM精确定时 ④残余CFO ⑤BEM+Turbo(10轮)
（训练精估替代CP精估，因SC-TDE无CP结构）
...
**V5.0→V5.1**: 修复LFM检测——标称峰值窗口搜索(static@5dB从50%→1.95%)
**static**: 0%@10dB+，5dB尚可（1.95%）
**fd=1Hz**: 多普勒估计误差88%，15dB可达0.76%但20dB反弹(1.60%疑似nv_post问题)
**fd=5Hz**: ~45%物理极限
**待优化**: 时变跳过训练精估(对齐OFDM策略) + nv_post兜底
...
### ✅ P2→V4.3: OFDM — 鲁棒架构固化
- V4.1: GAMP→OMP | V4.2: nv_post兜底 | V4.3: 跳过CP精估+空子载波CFO+DD-BEM
- static: 0%@5dB+, fd=1Hz: ~1%@15dB+(BEM极限), 对N_blocks/参数鲁棒

### 🔶 P3→V5.1: SC-TDE — LFM同步修复，fd=1Hz待优化
- V5.0→V5.1: 修复LFM检测(标称峰值窗口搜索替代全局搜索)
- static: 1.95%@5dB, 0%@10dB+（修复前50%→1.95%）
- fd=1Hz: 0.76%@15dB，但多普勒估计误差88%，20dB反弹(待nv_post兜底)
- fd=5Hz: ~45%物理极限
...
| ✅ | **OFDM端到端V4.3** | **完成** | OMP+nv_post+跳过CP+null SC CFO+DD-BEM, 鲁棒架构 |
| 🔶 进行 | **SC-TDE端到端V5.1** | **LFM修复验证** | static@5dB:1.95%, fd=1Hz@15dB:0.76%, 待优化时变多普勒 |
...
6. **SC-FDE/OFDM/SC-TDE均已去oracle**：SC-FDE V4.0+OFDM V4.3+SC-TDE V5.1均为盲估计
