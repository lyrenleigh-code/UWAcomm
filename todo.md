...
| **OTFS 通带 2D 脉冲整形 Phase 4** | 🔴 Phase 4 BER FAIL（2026-05-04） | spec `active/2026-04-13-otfs-pulse-shaping.md`；diag `tests/OTFS/diag_phase4_hann_ber.m`：rect / hann × {static, disc-5Hz, hyb-K5} × SNR={10,15} dB；hann BER 全 6 trial 退化 +1.9~+14.8 pp（acceptance #4 0/6 PASS）；loopback 误差 hann 2.78e+01 vs rect 1.26e-15 → spec 假设"RX 不做窗补偿，窗效应由均衡器自然吸收"被证伪；**决议：维持 rect 默认部署**，Phase 0-2 PSL 改善 13.8 dB 保留作设计层理论参考；重启路径 = 改 `otfs_demodulate` + `eq_otfs_lmmse` 传 `pulse_type`，再 Phase 4 重跑 |
| ~~**OTFS PAPR 专项降低**~~ | ✅ 2026-04-27 漏登（2026-05-04 补登） | commit `c9c0601`，移植 codex 已 completed spec；`otfs_slm_select.m` 新增（SLM 16 候选 min-PAPR）+ `papr_clip.m` clip16/SLM；test_multicarrier 4.3 SLM PASS（PAPR 6.6→5.8 dB）；spread pilot PAPR 主路径见下行 |
| ~~**OTFS 扩散 pilot**~~ | ✅ 2026-04-27 漏登（2026-05-04 补登） | commit `c9c0601`，spec `active/2026-04-14-otfs-spread-pilot.md` Result 段补；superimposed pilot 落地（方案 C），`ch_est_otfs_superimposed.m` 新增；test_multicarrier 4.4 PASS：impulse PAPR 16.8 dB → superimposed **8.9 dB**（达成 spec ≤10 dB 目标）；数据率 5357→5902 bps (+10%)；5/6 fading SNR≥10 dB BER 全 0%；known limitation：jakes5Hz 灾难（impulse 33-35% / superimposed 42-44% 连续谱物理 limitation） |
...
| 任务 | 完成日期 | 备注 |
|------|---------|------|
| **SC-FDE V4.1 高 SNR cascade BEM/GAMP 灾难修复** | **2026-05-04** | spec `2026-05-04-scfde-high-snr-cascade-bem-disaster.md`；commit `7cd0ed7`；`modem_decode_scfde.m` 三处 V4.1 patch（L131/L174 nv_eq clamp 到 sig_pwr × 1e-3 = 30dB SNR floor 抑制 GAMP nv_post→0 数值发散；L183 trigger_pretturbo 高 SNR>25dB 时 disable 退化 V3.0 GAMP 静态信道避免 BEM 过拟合 noise）。SNR sweep 实测：pass 50.23%→**0.43%** (117×) / SNR=80 48.71%→0.53% (94×) / SNR=10/20 0% 不变 / SNR=30 20.88%→5.47% (4×)；5/5 接受准则 PASS；完整矩阵 24/24 解码成功，20/24 BER<5%（vs 修复前 19/24，+1）；其他 5 体制 BER 完全不变。残余：SNR 25→30 dB 5.47pp 跳变（边界效应，fd_est_pretturbo 平滑过渡为 follow-up）。修复主导机制 A（GAMP 数值发散）+ B（BEM 过拟合）。|
| **TX/RX 简化 UI 拆分 v2.0（jakes passband-native + OTFS fix + 24/24 矩阵 PASS + 详细测试报告）** | **2026-05-04** | spec `2026-05-04-tx-rx-simple-ui-split.md` + plan；commits `0d83462` MVP + `a2b9869` v2.0；用户决策"放弃 P4 UI 复杂架构"后落地 5 文件 + 9 测试/diag。**TX UI** 6/6 体制 PASS（生成 wav+JSON+base64 known_bits+encode_meta 白名单透传）；**RX UI** classdef + uifigure 同步回调（无 timer/closure）+ 流式 chunk-by-chunk + 4 信道模式（pass/awgn/jakes/multipath）。V2.0 改动：apply_jakes_full passband-native（Hilbert+per-tap SoS Jakes envelope，fs_pos +8 vs V1.0 +7328 改善 916×）；OTFS K×2 维度 fix（JSON round-trip 把 [16,32] 解码为 2×1 column → modem_decode_otfs L52 oob，加 local_fix_otfs_meta_dims 校正 pilot_info/data_indices/guard_mask/dd_frame）；audio_in 末尾 0.2s silence pad；try_decode 总调 comp_resample 统一 GATE/COMP；所有模式 dither -80dB。完整矩阵 6×4=24 cases，**24/24 解码成功**，19/24 BER<5%。**4 已知 limitation** 量化：F1 SC-FDE V4.0 高 SNR 灾难（已 V4.1 修复）/ F2 jakes 38-50%（fs_pos+8 sync 偏差）/ F3 jakes fs_pos+8 / F4 ring 边界。**测试报告** `wiki/modules/14_Streaming/simple-ui-test-report.md`（9 段：BER 矩阵 / α gate / fs_pos / 耗时 / RCA / API / 推荐场景 / follow-up / 文件清单）。|
...
| **OTFS PAPR + 扩散 pilot（commit `c9c0601` 漏登补登）** | **2026-04-27**（2026-05-04 补登） | commit `c9c0601` 移植 codex 已 completed spec：`otfs_slm_select.m` SLM 16 候选 + `papr_clip.m` clip16 + `otfs_pilot_embed.m` superimposed 模式 + `ch_est_otfs_superimposed.m`；test_multicarrier 24/24 PASS（4.3 SLM PASS 6.6→5.8 dB / 4.4 spread pilot impulse 16.8 dB → superimposed **8.9 dB**）；2 spec Result 段 2026-05-04 补；known limitation：jakes5Hz 灾难（连续谱物理 limit） |
| **OTFS 通带 2D 脉冲整形 Phase 4 BER 验证 — FAIL** | **2026-05-04** | diag `tests/OTFS/diag_phase4_hann_ber.m`；hann × {static, disc-5Hz, hyb-K5} × SNR={10,15} dB 全 6 trial BER 退化 +1.9~+14.8 pp；loopback 误差 hann 2.78e+01 vs rect 1.26e-15 → spec 假设"窗效应由均衡器自然吸收"证伪；**决议**：维持 rect 默认部署，Phase 0-2 PSL 改善 13.8 dB 保留作设计层理论参考；spec `active/2026-04-13-otfs-pulse-shaping.md` Result 段补 Phase 4 实测 + FAIL 决议；重启路径预留（改 `otfs_demodulate` + `eq_otfs_lmmse` 传 `pulse_type`） |
