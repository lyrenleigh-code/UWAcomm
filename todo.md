...
| **SC-TDE fd=1Hz α estimator fix（V5.5 partial）** | 🟡 partial 2026-04-25 | spec `active/2026-04-25-sctde-fd1hz-alpha-estimator-fix.md`（保留 active，等用户判断后续）；Phase 1.2 量化（L0 deterministic +1.5e-5）+ Phase 2 ablation（**R3 排除/sub-sample 必需，R5 confirmed 新机制 iter 反向收敛，R1 部分支持/LFM peak deterministic shift**）；fix runner V5.5（fd-conditional iter default）；verify_alpha_sweep 通过；**3/5 接受准则 PASS**（SNR=15 全准则 + 单调性恢复 4.33→4.55→2.97→2.55）+ **2/5 partial**（SNR=20 mean 4.55→2.55% vs ≤1.5%、灾难率 33.3% vs ≤15%）；残余 1.66% gap 由 L0 estimator deterministic +1.5e-5 bias 解释（iter=0 已是层内最优）；下一步候选：多 LFM ensemble / Jakes-aware estimator（独立 spec）+ estimator-外灾难调研（类比 SC-FDE Phase J） |
| **L0 deterministic α bias 校正（多 LFM ensemble / Jakes-aware estimator）** | 🆕 候选 spec | V5.5 残余 +1.5e-5 deterministic bias，候选方向：HFM+/HFM- 第二组 dual-chirp 平均 / LFM 模板 Jakes-aware 鲁棒化；预期收益 SNR=20 mean 2.55%→~1-1.5%；待 spec 立项 |
| **SC-TDE fd=1Hz estimator-外灾难调研** | 🆕 候选 spec | V5.5 + oracle 下 SNR=20 仍 1/15（s15=8.90%）灾难——非 estimator 偏差驱动，类比 SC-FDE Phase J；候选机制：BEM/Turbo/CFO 稀有触发；待 spec 立项 |
