...
| 指标 | 数值 | 说明 |
|------|------|------|
| MATLAB 函数文件 | 393 个 | 含测试 / 可视化 / 辅助 |
| 代码总行数 | 62,399 行 | 2026-05-01 统计 |
...
\*OTFS 在离散 Doppler 信道(含分数频移, max 5Hz)下实现 0% BER@10dB+；Jakes 连续谱信道 fd=5Hz 33-44% 灾难（已知 limitation，需协议层改动）；spread-pilot PAPR 16.8→8.9dB。

> **2026-04-26 SC-FDE Phase 4+5 协议突破**：cfg.pilot_per_blk=128 → fd=1Hz BER 47%→3.37% (14×)，需 blk_cp ≠ blk_fft setup（runner 验证；UI 默认 blk_cp=blk_fft 不自动激活）。
