...
- 2026-04-19: Spec 创建
- 2026-04-19: Step 1/2/3 全部完成

## Result

✅ **已完成**。3 步实施成功：

### Step 1: modem_encode_otfs V2.0.0
- otfs_modulate 输出符号域 otfs_signal @ sym_rate (N_otfs_sym = 3072)
- pulse_shape(otfs_signal, sps=8, 'rrc', 0.35, 6) → body_bb @ fs=48000
- meta 新增字段：`N_otfs_sym` / `sps` / `rolloff` / `span` 供 RX 下采样
- sys_params_default 新增 `sys.otfs.rolloff=0.35` / `sys.otfs.span=6`

### Step 2: modem_decode_otfs V2.0.0
- match_filter 匹配滤波 body_bb @ fs → rx_filt
- 符号定时搜索：本地重生成 pilot-only DD 帧 → otfs_modulate 得 pilot_ref_sym，
  用作定时参考（RX 可独立重生成，不违反 Oracle）
- 遍历 sps 个 offset 取 |corr| 最大 → best_off
- rx_filt(best_off+1 : sps : end) → rx_sym @ sym_rate 长度对齐 N_otfs_sym
- otfs_demodulate(rx_sym, N, M, cp_len, 'dft') → Y_dd
- info 新增 sym_off_best / sym_off_corr / sym_off_best_val

### Step 3: UI 恢复 + 验证
- p3_demo_ui.m scheme_dd 恢复 'OTFS (DD域 + LMMSE/MP)' 选项
- UI 启动 OK
- 全量回归：test_p3_unified_modem 2/2 + test_p3_2_ofdm_sctde 2/2 +
  test_p3_3_dsss_otfs 3/3 = **7/7 PASS**

### 关键数字

| 指标 | V1.0 | V2.0 |
|------|------|------|
| body_bb 采样率 | sym_rate=6000 | fs=48000 ✓ |
| body_bb 长度 | 3072 | ~24608 |
| 与其他 5 体制接口统一 | ❌ | ✅ |
| P3 UI 可用 | ❌ | ✅ |

### 未做（spec 未要求）

- `test_sync.m` OTFS 帧回环（可选）
- UI 下实际 Transmit 成功率 BER 仿真验证（需用户手动）
