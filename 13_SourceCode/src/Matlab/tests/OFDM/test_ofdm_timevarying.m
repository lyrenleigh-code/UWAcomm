...
% 信道: 基带复数卷积(Jakes衰落+全局多普勒) → 上变频+实噪声 → 下变频
% RX: 已知α补偿 → RRC匹配 → 去CP+FFT → oracle H_est(块中点) → MMSE → 跨块BCJR
% 测试矩阵: fd=[0,1,5]Hz × SNR=[5,10,15,20]dB × 块长扫描
% 版本：V1.0.0
    ...
    for si = 1:length(snr_list)
        ...
        for bi = 1:N_blocks
            ...
            ch_params = struct('fs',fs,'delay_profile','custom',...
                ...
                'snr_db',Inf,'seed',200+fi*100+si*10+bi);  % 先不加噪
            ...
            rng(300+fi*1000+si*100+bi);  % 噪声种子
            ...
            for off=0:sps-1
                ...
                if length(st)>=10, c=abs(sum(st(1:10).*conj(x_cp(1:10))));
...
title(sprintf('信道CIR（%d径, max=%.0fms）', length(sym_delays), max(delays_ms)));
grid on;
...
plot(f_khz, 20*log10(abs(H_show)+1e-10), 'b', 'LineWidth',1);
