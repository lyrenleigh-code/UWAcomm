...
% OTFS 暂未接入 P3 demo UI：modem_encode_otfs 输出符号域 body_bb（采样率 sym_rate），
% 与 assemble_physical_frame 的 fs 采样率 preamble 不兼容（需 up/down sample 桥接）。
% 见 spec 2026-04-19-p3-otfs-sampling-bridge.md（待创建）。
% 命令行 loopback 测试 test_p3_3_dsss_otfs.m 仍可用（走基带直通）。
