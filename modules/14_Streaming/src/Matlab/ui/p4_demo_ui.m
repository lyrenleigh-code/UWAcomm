...
app.tx_h_tap        = [];
app.tx_alpha_true   = [];         % P4: 真实 α(t) 序列（gen_doppler_channel 输出）
...
%% ---- 主 figure ----
app.fig = uifigure('Name', '通信声纳 · Streaming P4 Demo (真实多普勒)', ...
...
function build_topbar(main)
    ...
    app.sonar_badge = p4_sonar_badge(badge_wrap);
...
function build_middle_panels(main)
    ...
    app.tv_enable_cb.ValueChangedFcn = @(~,~) on_tv_model_changed();
    [~, app.tv_model_dd]    = mk_row(tx_grid, 7, '时变模型:', 'dropdown', ...
        'random_walk', {'constant','linear_drift','sinusoidal','random_walk'});
    app.tv_model_dd.ValueChangedFcn = @(~,~) on_tv_model_changed();
    ...
    lbl_fad.Layout.Row = 10; lbl_fad.Layout.Column = 1;
    app.fading_dd = uidropdown(tx_grid, ...
        'Items', {'static (恒定)', 'slow (Jakes 慢衰落)', 'fast (Jakes 快衰落)'}, ...
        'Value', 'static (恒定)');
    app.fading_dd.Layout.Row = 10; app.fading_dd.Layout.Column = 2;

    [~, app.jakes_fd_edit] = mk_row(tx_grid, 11, 'Jakes fd (Hz):', 'numeric', 2, [0 20]);

    lbl_pre = uilabel(tx_grid, 'Text', '信道预设:');
    lbl_pre.Layout.Row = 12; lbl_pre.Layout.Column = 1;
    app.preset_dd = uidropdown(tx_grid, ...
        'Items', {'AWGN (无多径)', '6径 标准水声', '6径 深衰减', '3径 短时延'}, ...
        'Value', '6径 标准水声');
    app.preset_dd.Layout.Row = 12; app.preset_dd.Layout.Column = 2;
    ...
    card_ber  = p4_metric_card(ber_grid, '比特 BER',   '—',     '',      'primary');
    card_err  = p4_metric_card(ber_grid, '错误 / 总',  '—',     'bits',  'muted');
    card_fifo = p4_metric_card(ber_grid, 'FIFO',       '0',     '样本',   'accent');
    card_det  = p4_metric_card(ber_grid, '检测状态',    '空闲',  '',      'muted');
    % 长文本字段：降小字号避免溢出
...
% P4: type 扩展为 'numeric'（原始）/ 'checkbox' / 'dropdown'
%   numeric:  lim = [lo hi]
%   checkbox: val = true/false, lim 忽略
%   dropdown: val = 默认字符串, lim = cell array of items
    lbl = uilabel(g, 'Text', label, 'FontColor', PALETTE.text);
    lbl.Layout.Row = row; lbl.Layout.Column = 1;
    switch type
        case 'numeric'
            edt = uieditfield(g, 'numeric', 'Value', val, 'Limits', lim, ...
                'ValueDisplayFormat', '%g', ...
                'BackgroundColor', PALETTE.surface_alt, 'FontColor', PALETTE.text);
        case 'checkbox'
            edt = uicheckbox(g, 'Value', logical(val), 'Text', '', ...
                'FontColor', PALETTE.text);
        case 'dropdown'
            edt = uidropdown(g, 'Items', lim, 'Value', val, ...
                'BackgroundColor', PALETTE.surface_alt, 'FontColor', PALETTE.text);
        otherwise
            error('mk_row: 不支持的 type: %s', type);
    end
...
function on_tv_model_changed()
    % P4: 时变多普勒控件状态切换
    % - enable=false 或 model='constant' → 灰掉 drift/jitter 编辑框
    is_const = ~logical(app.tv_enable_cb.Value) || strcmp(app.tv_model_dd.Value, 'constant');
    app.tv_drift_edit.Enable  = ~is_const;
    app.tv_jitter_edit.Enable = ~is_const;
    if is_const
        append_log('[TV] 时变多普勒关闭（constant 模式，α 全程恒定）');
    else
        append_log(sprintf('[TV] 时变模型 → %s (drift=%.2g µ/s, jitter=%.2g µ)', ...
            app.tv_model_dd.Value, app.tv_drift_edit.Value, app.tv_jitter_edit.Value));
    end
end
...
function on_transmit()
    try
        ...
        if app.bypass_rf
            ...
            app.tx_signal_start = app.fifo_write + 1;
            app.tx_pending      = true;
            bw_tx = p4_downconv_bw(sch, app.sys);
...
function try_decode_frame()
    ...
    alpha_est_rx = 0;
    if isfield(sync_det, 'alpha_est')
        alpha_est_rx = sync_det.alpha_est;
    end
    alpha_conf = 0;
    if isfield(sync_det, 'alpha_confidence')
        alpha_conf = sync_det.alpha_confidence;
    end
    append_log(sprintf('[SYNC] fs=%d gt=%d diff=%+d peak=%.1f ratio=%.1f conf=%.2f α=%+.3e conf_α=%.2f', ...
    ...
    p4_render_tabs(sch, entry, app.tabs, app.sys, app.history, app.style);
...
function on_history_select()
    ...
    p4_render_tabs(entry.scheme, entry, app.tabs, app.sys, app.history, app.style);
    % 也更新 info 面板
