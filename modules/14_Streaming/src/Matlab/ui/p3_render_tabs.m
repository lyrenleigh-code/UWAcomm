...
%   - draw_unit_circle 作为本文件 local function
    ...
    % --- 质量历史 tab ---
    try
        p3_render_quality(history, struct( ...
    ...
    % --- 同步/多普勒 tab ---
    try
        p3_render_sync(entry, history, struct( ...
...
function render_channel_multipath(sch, entry, ax_td, ax_fd, sys, P)
    ...
    hold(ax_td, 'off');
    title(ax_td, '时域 CIR  ·  真实(渐变) vs 估计(红)', 'Color', P.primary_hi);
    if has_est_td
