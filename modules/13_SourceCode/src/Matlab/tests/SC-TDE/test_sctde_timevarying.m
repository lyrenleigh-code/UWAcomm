...
alpha_lfm_raw_matrix  = nan(size(fading_cfgs,1), length(snr_list));   % 2026-04-25 fd=1Hz α fix Phase 1
alpha_lfm_iter_matrix = nan(size(fading_cfgs,1), length(snr_list));
alpha_lfm_scan_matrix = nan(size(fading_cfgs,1), length(snr_list));
% 2026-04-25 Phase 2 C2：LFM 峰诊断
diag_tau_up_int_matrix   = nan(size(fading_cfgs,1), length(snr_list));
diag_tau_dn_int_matrix   = nan(size(fading_cfgs,1), length(snr_list));
diag_tau_up_frac_matrix  = nan(size(fading_cfgs,1), length(snr_list));
diag_tau_dn_frac_matrix  = nan(size(fading_cfgs,1), length(snr_list));
...
for fi = 1:size(fading_cfgs,1)
    ...
    for si = 1:length(snr_list)
        ...
        if exist('bench_use_subsample','var') && ~isempty(bench_use_subsample)
            cfg_alpha.use_subsample = logical(bench_use_subsample);
        else
        ...
        alpha_lfm_raw_snapshot = alpha_lfm;   % L0：est_alpha_dual_chirp 直出
        % 2026-04-25 Phase 2 C2：暴露 LFM 峰诊断字段
        diag_tau_up_int_snap   = alpha_diag.tau_up;
        diag_tau_dn_int_snap   = alpha_diag.tau_dn;
        diag_tau_up_frac_snap  = alpha_diag.tau_up_frac;
        diag_tau_dn_frac_snap  = alpha_diag.tau_dn_frac;
        ...
        if exist('bench_alpha_iter','var') && ~isempty(bench_alpha_iter)
            eff_iter = bench_alpha_iter;       % caller explicit override
        elseif fd_hz == 1 && ~strcmpi(ftype, 'static')
            eff_iter = 0;                       % fd=1Hz Jakes 默认关 iter
        else
            eff_iter = 2;                       % 其他场景保留 V5.4 default
        end
        if eff_iter > 0 && abs(alpha_lfm) > 1e-10
            for iter_a = 1:eff_iter
        ...
        alpha_lfm_iter_snapshot = alpha_lfm;  % L1：iter refinement 后
        ...
        alpha_lfm_scan_snapshot = alpha_lfm;  % L2：大 α 局部精扫后
        ...
        alpha_lfm_raw_matrix(fi,si)  = alpha_lfm_raw_snapshot;
        alpha_lfm_iter_matrix(fi,si) = alpha_lfm_iter_snapshot;
        alpha_lfm_scan_matrix(fi,si) = alpha_lfm_scan_snapshot;
        diag_tau_up_int_matrix(fi,si)   = diag_tau_up_int_snap;
        diag_tau_dn_int_matrix(fi,si)   = diag_tau_dn_int_snap;
        diag_tau_up_frac_matrix(fi,si)  = diag_tau_up_frac_snap;
        diag_tau_dn_frac_matrix(fi,si)  = diag_tau_dn_frac_snap;
...
if benchmark_mode
    ...
    for fi_b = 1:size(fading_cfgs,1)
        for si_b = 1:length(snr_list)
            ...
            row.alpha_lfm_raw    = alpha_lfm_raw_matrix(fi_b, si_b);   % 2026-04-25 fd=1Hz α fix Phase 1
            row.alpha_lfm_iter   = alpha_lfm_iter_matrix(fi_b, si_b);
            row.alpha_lfm_scan   = alpha_lfm_scan_matrix(fi_b, si_b);
            row.diag_tau_up_int   = diag_tau_up_int_matrix(fi_b, si_b);   % Phase 2 C2
            row.diag_tau_dn_int   = diag_tau_dn_int_matrix(fi_b, si_b);
            row.diag_tau_up_frac  = diag_tau_up_frac_matrix(fi_b, si_b);
            row.diag_tau_dn_frac  = diag_tau_dn_frac_matrix(fi_b, si_b);
