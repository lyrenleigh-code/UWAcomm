...
模块6位于符号映射/扩频之后（OFDM/SC-FDE/OTFS体制启用，其余跳过）。发端：输入为频域符号(OFDM)、时域符号块(SC-FDE)或DD域符号(OTFS)，经域变换+CP插入后输出时域信号。收端：输入为时域接收信号，经去CP+逆变换后输出频域/DD域信号供信道估计均衡使用。含PAPR计算和抑制功能。
