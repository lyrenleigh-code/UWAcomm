...
updated: 2026-04-25
---
<!-- 2026-04-25 SC-TDE V5.5 partial fix 章节追加 -->
...
## SC-TDE fd=1Hz V5.5 partial fix（2026-04-25，详见 [[../specs/active/2026-04-25-sctde-fd1hz-alpha-estimator-fix]]）

H4 confirmed 后续 fix。runner 暴露 4 层 α + LFM peak 7 字段，Phase 1.2 量化 + Phase 2 三条假设并行 ablation。

### Phase 数据要点（fd=1Hz, dop_rate=8.33e-5）

| 现象 | 事实 |
|------|------|
| L0 偏差 (mean \|err\|) | 1.45e-5 → 1.52e-5（SNR=10/15/20）— **deterministic，不随 SNR 变** |
| L1 iter 偏差 | **2.96e-5（base 翻倍）** — iter refinement 累加 deterministic bias 朝错误方向 |
| sub-sample off vs on | 5.7× 恶化（dtau 真值 μs 级被 1/fs ≈ 21μs 量化掉）→ **sub-sample 必需，R3 排除** |
| LFM tau_up_frac | 系统偏向 +0.44（接近 +0.5），三 SNR 一致 → **R1 部分支持**（Jakes 时变让 LFM 模板 deterministic peak shift）|
| bad vs good seed err 差 | 仅 0.26e-5（约 3% 真值），估计偏差 **非灾难驱动**——灾难来自 estimator-外机制（类比 SC-FDE Phase J）|

### V5.5 fix（runner，最小侵入）

`test_sctde_timevarying.m`：`fd=1Hz Jakes` 默认 `bench_alpha_iter=0`，其他场景保留 V5.4 default 2，caller explicit set 仍优先。estimator API 不动；其他 4 体制 runner 不动。

`verify_alpha_sweep` 通过：α 工作范围 ±1e-3..±1e-2 全 0%，α=+3e-2 49.86% 物理极限保留。

### V5.5 三方对比（fd=1Hz × 15 seed × 3 SNR）
...
- V5.5 SNR=20 mean 2.55% 与 oracle 0.89% gap 1.66% **由 L0 deterministic +1.5e-5 bias 解释**（estimator 物理上界，iter=0 已是层内最优 partial fix）
- 灾难率 33% 中 1/15（s15=8.90%）即 oracle 残留（estimator-外机制），其余 4 个由 estimator 偏差驱动
...
### BER 分布（α=+1e-2，SNR=10 dB，15 seed）

| 体制 | median | max | 灾难率 (BER>30%) | 状态 |
|------|:------:|:---:|:---------------:|:---:|
| **OFDM** | 0% | **0.13%** | 0/15 | ✅ **最健康**（反直觉）|
| SC-FDE | 0% | 29% | 0/15 | ✓ 健康（Phase J 6.7% 可能是 seed 1..30 子集效应）|
| FH-MFSK | 0% | 1.4% | 0/15 | ✓ 健康 |
| **SC-TDE** | **49.6%** | **51.2%** | **15/15 (100%)** | ❌ **全灾难** |
| **DSSS** | **46.2%** | **50.2%** | **15/15 (100%)** | ❌ **全灾难** |

### 关键发现

1. **SC-TDE + DSSS 在 α=+1e-2 下 100% 完全不可用**（不是"敏感"或"限制"，是彻底崩溃）
2. **OFDM 最健康**（max 0.13%）— 与直觉相反（CP-OFDM 在多径下反而最稳）
3. **SC-FDE 0/15 灾难**：Phase J Monte Carlo 的 6.7% 灾难率可能是 seed 1..30 子集里有敏感点，seed 1..15 未命中；需更大样本确认真实率

### 与历史规划的一致性修正

旧 todo 记录"DSSS α≥1e-2 扩频 chip-timing 固有限制"和"SC-TDE 下游 α 敏感"都是**定性描述**。Phase c 首次**定量证明**：
- DSSS α=+1e-2 ≈ **完全不工作**（不是"部分降级"）
- SC-TDE α=+1e-2 ≈ **完全不工作**（不是"性能下降"）

### 对生产系统真实 α 工作范围

修正旧的"6 体制全能跑 α=3e-2"虚报：

| 体制 | α 工作上限 |
|------|:---------:|
| OFDM / FH-MFSK | 3e-2（45m/s 鱼雷 / 高速 AUV）✓ |
| SC-FDE | 3e-2（但 seed-sensitive，实际 1-10% 灾难率视 seed 分布而定）|
| **SC-TDE / DSSS** | **< 1e-2**（≥ 1e-2 即完全死机）|

### 后续独立 spec 待开

- **SC-TDE α 深挖**：哪层导致 100% 灾难？（2-3h）
- **DSSS α 深挖**：Sun-2020 符号级跟踪在 α=+1e-2 下失效根因（已部分归档 `2026-04-22-dsss-symbol-doppler-tracking`，需扩 adaptive Gold31 bank）
...
### OMP 替换实验（2026-04-23 spec `2026-04-23-scfde-omp-replace-gamp-and-oracle-clean`）

试用 `ch_est_omp(y, Phi, L_h, K_sparse=6, nv_eq)` 替代 GAMP V1.4：

| 估计器 | -1e-2 灾难率 | -1e-2 max | +1e-2 灾难率 | +1e-2 max |
|---|:-:|:-:|:-:|:-:|
| **GAMP V1.4** | 0% | 21.2 | 6.7% | 30.6 |
| **OMP K=6** | 0% | 19.2 ✓ | **10% ↗** | 32.1 |

**结论**：OMP 在 SNR=10 边界 case 反而比 GAMP+LS 略差（+1e-2 灾难率涨 1 个 case）。Phase 0 假设"OMP 直接给稀疏 6 径解，不会过拟合"被证伪 — OMP K=6 强制选 6 个 column，残差驱动可能选错 support → 同样错估。
**保留 OMP 作 `tog.use_omp_static=true` toggle 便于复现实验，默认仍 V1.4 GAMP**。

### sps 相位选择去 oracle 失败（同 spec）

L484-487 + L590-596 用 `all_cp_data(1:10)` 做相关挑 sps 相位（oracle 泄漏）。试用功率最大化 `sum(|st|²)` 替代：

| 改法 | -1e-2 BER | +1e-2 灾难率 |
|---|:-:|:-:|
| baseline 相关 | 13% | 6.7% |
| **功率最大化** | **48%** ❌ | 10% |

**根因**：custom6 6 径有显著 ISI，RRC 脉冲尾部能量泄漏到邻近符号。错误 sps 相位反而捕获更多 ISI 能量 → 选错相位 → BER 退化。教科书功率最大化只在单径无 ISI 信道有效。

**后续真去 oracle 需另起 spec**：用 LFM 模板/training preamble 做相关（RX-known reference），或加 Gardner TED 量化到 sps offset，或改帧结构加 training。
