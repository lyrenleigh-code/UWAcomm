...
开发进度：`todo.md`
调试记录：`wiki/debug-logs/{模块名}/` (按模块分文件夹)
...
- **每次提交同步更新调试日志并清理txt**：每次git commit后，必须同步更新项目 wiki 中的调试日志，记录本次变更内容、测试结果和关键发现；同时**删除测试目录下的临时txt结果文件**（`test_*_results.txt`等调试产物，不纳入版本控制）。日志存放规则：
  - 路径：`wiki/debug-logs/{模块名}/`，如 `wiki/debug-logs/08_Sync/`、`wiki/debug-logs/13_SourceCode/`
  - 模块文件夹不存在时**直接新建**
  - **调试日志合并策略（关键）**：不再按日期创建独立文件。每个体制和每个模块各维护**一个累积调试日志**，新的调试记录作为带日期的章节**追加到对应日志末尾**。例如：
    - 端到端体制日志：`wiki/debug-logs/13_SourceCode/SC-FDE调试日志.md`
    - 模块日志：`wiki/debug-logs/08_Sync/同步调试日志.md`
  - **双向链接**：调试日志中使用 `[[wikilink]]` 链接相关概念页和模块页
  - **标签必须**：每个笔记 frontmatter 包含 `tags: [调试日志, SC-FDE]` 等
  - 跨模块变更放在主要涉及的模块文件夹下
  - 跨项目有价值的调试经验用 `/promote` 回流 Hub
...
12. **OTFS PAPR无法窗化降低(2026-04-13)**：PAPR=7.1dB根因IFFT随机叠加，CP-only和数据脉冲成形均无效
13. **Hann脉冲成形降旁瓣有效(2026-04-13)**：频谱PSL降13.8dB，模糊度多普勒PSL降33dB，分辨力展宽2.3x(水声可接受)
...
| OTFS通带2D脉冲整形 | 🔶Phase2完成 | Hann旁瓣降13.8dB, 模糊度PSL降33dB; PAPR无法窗化降低; 待E2E BER验证 |
| OTFS两级同步架构 | 🔴待做 | 对齐其他体制HFM+LFM帧结构 |
| SC-TDE fd=1Hz优化 | 🔶待优化 | nv_post兜底+时变跳过训练精估 |
| 离散Doppler全体制对比 | ✅完成 | apply_channel提取到common, 6体制×6信道BER矩阵 |
| OTFS PAPR专项降低 | 🟡待做 | 需SLM/PTS/削峰, 窗化无法解决(根因IFFT叠加) |
...
```
01-spec → 02-plan → 03-implement(产出三件套: 代码+测试+README) → 04-validate(验证+同步wiki+归档+commit)
```
